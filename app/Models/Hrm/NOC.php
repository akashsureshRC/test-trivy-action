...
class NOC extends Model
{
    ...
    public static function defaultNocCertificate($company_id = null, $workspace_id = null)
    {
        $defaultTemplate = [
        ...
        foreach ($defaultTemplate as $lang => $content) {
            $NocCertificate = NOC::where('lang', $lang)->where('workspace', $workspace_id)->where('created_by', $company_id)->first();
            if (empty($NocCertificate)) {
                NOC::create(
                    [
                        'lang' => $lang,
                        'content' => $content,
