...
class EmployeeWorkingHour extends Model
{
    ...
    /**
     * Get the employee that owns this working hour
     */
    ...
    /**
     * Create default working hours for an employee (Mon-Fri 08:00-17:00)
     */
    public static function createDefaultForEmployee(int $employeeId): void
    {
     ...
     */
    public static function copyFromBranch(int $employeeId, int $branchId): void
    {
        $branchHours = BranchWorkingHour::where('branch_id', $branchId)->get();

        foreach ($branchHours as $bh) {
            self::updateOrCreate(
                ['employee_id' => $employeeId, 'day' => $bh->day],
                [
                    'is_working_day' => $bh->is_working_day,
                    'start_time' => $bh->start_time,
                    'end_time' => $bh->end_time,
                    'lunch_start_time' => $bh->lunch_start_time,
                    'lunch_end_time' => $bh->lunch_end_time,
                ]
            );
        }
    }
