...
class ExperienceCertificate extends Model
{
    ...
    public static function defaultExpCertificat($company_id = null, $workspace_id = null)
    {
        $defaultTemplate = [
        ...
        foreach ($defaultTemplate as $lang => $content) {
            $ExpCertificat = ExperienceCertificate::where('lang', $lang)->where('workspace', $workspace_id)->where('created_by', $company_id)->first();
            if (empty($ExpCertificat)) {
