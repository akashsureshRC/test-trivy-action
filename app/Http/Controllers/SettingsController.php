...
class SettingsController extends Controller
{
    ...
    {
        // Restrict access for payroll_officer role
    ...
    public function getSettingSection($module, $method = 'index')
    {
        // Restrict access for payroll_officer role
        if (Auth::user()->type === 'payroll_officer') {
            return response()->json(['status' => 403, 'error' => __('Permission denied.')], 403);
        }
    ...
    {
        // Get all email notifications (simplified - no module separation for payroll app)
        $email_notify = Notification::where('type', 'mail')->get(['module', 'action', 'permissions']);
        $email_setting = EmailTemplate::$email_settings;
        return view('email.index', compact('settings', 'email_notify', 'email_setting'));
    }
    ...
    public function sendTestMail(Request $request)
    {
        ...
        if($validator->fails())
        {
            ...
            return errorRes($messages->first());
        }
        try
        {
            config(
                [
                    'mail.default' => 'smtp',
                    'mail.mailers.smtp.transport' => $request->mail_driver ?? 'smtp',
            ...
            return successRes(__('Email send Successfully'));
