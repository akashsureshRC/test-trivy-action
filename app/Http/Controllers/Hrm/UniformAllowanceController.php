...
class UniformAllowanceController extends Controller
{
    /**
    ...
    public function create(Request $request)
    {
        ...
        return view('hrm.uniform-allowances.create',compact('employee','term'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('payroll.index', ['employee_id' => $request->employee_id,'term' => $term])
                    ->with('success', 'Uniform Allowance Local added successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $uniformAllowance =  UniformAllowance::findOrFail($id);
        $term = $request->query('term', $uniformAllowance->term); 
        return view('hrm.uniform-allowances.edit', compact('uniformAllowance','term'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $uniformAllowance->update($request->all());

        return redirect()->route('payroll.index', ['employee_id' => $uniformAllowance->employee_id,'term' => $uniformAllowance->term])
            ->with('success', 'Uniform Allowance updated successfully.');
    }
    ...
    {
        $uniformAllowance = UniformAllowance::findOrFail($id);
        $employee_id = $uniformAllowance->employee_id;
        $term = $uniformAllowance->term;
        $uniformAllowance->delete();

        return redirect()->route('payroll.index', ['employee_id' => $employee_id,'term' => $term])
            ->with('success', 'Uniform Allowance deleted successfully.');
    }
