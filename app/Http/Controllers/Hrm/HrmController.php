...
class HrmController extends Controller
{
    ...
    {
        if (moduleIsActive('GoogleAuthentication')) {
            $this->middleware('2fa');
        }
    }
    ...
    public function index(Request $request)
    {
        if (Auth::check()) {
            if (Auth::user()->isAbleTo('hrm dashboard manage')) {
                $user = Auth::user();
                $events = [];
                $holidays = \App\Models\Hrm\Holiday::where('created_by', '=', creatorId())->where('workspace', getActiveWorkspace());
                ...
                $hrm_events = \App\Models\Hrm\Event::where('created_by', '=', creatorId())->where('workspace', getActiveWorkspace());
                if (!empty($request->date)) {
                    $date_range = explode(' to ', $request->date);
