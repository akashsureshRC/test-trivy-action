...
use App\Models\Hrm\Event;
use App\Models\Hrm\EventEmployee;
use App\Events\Hrm\CreateEvent;
use App\Events\Hrm\DestroyEvent;
use App\Events\Hrm\UpdateEvent;
...
class EventController extends Controller
{
    ...
    public function index()
    {
        if (Auth::user()->isAbleTo('event manage')) {
            ...
            $employees = Employee::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkspace())->get();
    ...
    public function create()
    {
        if (Auth::user()->isAbleTo('event create')) {
            $employees   = Employee::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkspace())->get()->mapWithKeys(fn($e) => [$e->id => $e->first_name . ' ' . $e->last_name]);
    ...
    {
        if (Auth::user()->isAbleTo('Delete Event')) {
            if ($event->created_by == creatorId() && $event->workspace == getActiveWorkspace()) {
        ...
        if (isset($request->department_id)) {
            ...
            if (isset($request->department_id)) {
                if (in_array('0', $request->department_id)) {
                    $employees = Employee::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkspace())->get()->mapWithKeys(fn($e) => [$e->id => $e->first_name . ' ' . $e->last_name])->toArray();
                } else {
                    $employees = Employee::where('created_by', '=', creatorId())->where('workspace_id', getActiveWorkspace())->whereIn('department_id', $request->department_id)->get()->mapWithKeys(fn($e) => [$e->id => $e->first_name . ' ' . $e->last_name])->toArray();
                }
            }
        }
