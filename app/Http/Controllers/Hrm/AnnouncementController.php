...
use App\Models\Hrm\AnnouncementEmployee;
use App\Models\Hrm\Branch;
use App\Models\Hrm\Department;
use App\Models\Hrm\Employee;
use App\Events\Hrm\CreateAnnouncement;
use App\Events\Hrm\DestroyAnnouncement;
use App\Events\Hrm\UpdateAnnouncement;
    ...
    public function store(Request $request)
    {
        if (Auth::user()->isAbleTo('announcement create')) {
            ...
            if (in_array('0', $request->employee_id)) {
                $departmentEmployee = Employee::whereIn('department_id', $request->department_id)->where('workspace_id', getActiveWorkspace())->get()->pluck('id');
    ...
    public function getemployee(Request $request)
    {
        ...
        if (isset($request->department_id)) {
            if (in_array('0', $request->department_id)) {
                $employees = Employee::where('workspace_id', getActiveWorkspace())->get()->mapWithKeys(fn($e) => [$e->id => $e->first_name . ' ' . $e->last_name])->toArray();
            } else {

                $employees = Employee::where('workspace_id', getActiveWorkspace())->whereIn('department_id', $request->department_id)->get()->mapWithKeys(fn($e) => [$e->id => $e->first_name . ' ' . $e->last_name])->toArray();
            }
        }
