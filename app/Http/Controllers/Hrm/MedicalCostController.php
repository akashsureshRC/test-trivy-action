...
class MedicalCostController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('hrm.medical-costs.create',compact('employee','term'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('payroll.index', ['employee_id' => $request->employee_id,'term' => $term])
                    ->with('success', 'Medical Cost added successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        ...
        return view('hrm.medical-costs.edit', compact('medicalCost'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            'amount' => 'required|numeric|min:0',
            'medical_cost' => 'required|string',
        ]);

        $medicalCost->update($request->all());

        return redirect()->route('payroll.index', ['employee_id' => $medicalCost->employee_id,'term' => $medicalCost->term])
            ->with('success', 'Medical Cost updated successfully.');
    }
    ...
    {
        $medicalCost = MedicalCost::findOrFail($id);
        $employee_id = $medicalCost->employee_id;
        $term = $medicalCost->term;
        $medicalCost->delete();

        return redirect()->route('payroll.index', ['employee_id' => $employee_id,'term' => $term])
            ->with('success', 'Medical Cost deleted successfully.');
    }
