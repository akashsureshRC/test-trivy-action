...
class ToolAllowanceController extends Controller
{
    /**
    ...
    public function create(Request $request)
    {
        ...
        return view('hrm.tool-allowances.create',compact('employee','term'));
    }
        ...
        try {
           ...
           // $term = $request->term ?? $term;
            ...
            return redirect()->route('payroll.index', ['employee_id' => $request->employee_id,'term' => $term])
                    ->with('success', 'Tools Allowance Local added successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $toolAllowance = ToolAllowance::findOrFail($id);
         $term = $request->query('term', $toolAllowance->term);
        return view('hrm.tool-allowances.edit', compact('toolAllowance','term'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $toolAllowance->update($request->all());

        return redirect()->route('payroll.index', ['employee_id' => $toolAllowance->employee_id,'term' => $toolAllowance->term])
                    ->with('success', 'Tools Allowance updated successfully.');
    }
    ...
    {
        $toolAllowance = ToolAllowance::findOrFail($id);
        $employee_id = $toolAllowance->employee_id;
        $term = $toolAllowance->term;
        $toolAllowance->delete();

        return redirect()->route('payroll.index', ['employee_id' => $employee_id,'term' => $term])->with('success', 'Tools Allowance deleted successfully.');
    }
