...
class StaffPurchaseController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('hrm.staff-purchases.create',compact('employee','term'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('payroll.index', ['employee_id' => $request->employee_id,'term' => $term])
                    ->with('success', 'Staff Purchase Payments added successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $staffPurchase = StaffPurchase::findOrFail($id);
         $term = $request->query('term', $staffPurchase->term); 
        return view('hrm.staff-purchases.edit', compact('staffPurchase','term'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $staffPurchase->update($request->all());

        return redirect()->route('payroll.index', ['employee_id' => $staffPurchase->employee_id,'term' => $staffPurchase->term])
                    ->with('success', 'Staff Purchase Payments updated successfully.');
    }
    ...
    {
        $staffPurchase = StaffPurchase::findOrFail($id);
        $employee_id =  $staffPurchase->employee_id;
        $term =  $staffPurchase->term;
        $staffPurchase->delete();

        return redirect()->route('payroll.index', ['employee_id' => $employee_id,'term' => $term])
            ->with('success', 'Staff Purchase Payments deleted successfully.');
    }
