...
class TersPayoutController extends Controller
{
    /**
    ...
    public function create(Request $request)
    {
        ...
        return view('hrm.ters-payouts.create',compact('employee','term'));
    }
        ...
        try {
           ...
           $payslip = PaySlip::where('employee_id',$request->employee_id)->latest('id')->first();
            ...
            return redirect()->route('payroll.index', ['employee_id' => $request->employee_id,'term' => $term])
                    ->with('success', 'Ters payouts added successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        $tersPayout =  TersPayout::findOrFail($id);
        $term = $request->query('term', $tersPayout->term); 
        return view('hrm.ters-payouts.edit', compact('tersPayout','term'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $tersPayout ->update($request->all());

        return redirect()->route('payroll.index', ['employee_id' => $tersPayout->employee_id,'term' => $tersPayout->term])
                    ->with('success', 'Ters payouts updated successfully.');
    }
    ...
    {
        $tersPayout = TersPayout::findOrFail($id);
        $employee_id = $tersPayout->employee_id;
        $term = $tersPayout->term;
        $tersPayout->delete();

        return redirect()->route('payroll.index', ['employee_id' => $employee_id,'term' => $term])->with('success', 'Ters payouts deleted successfully.');
    }
