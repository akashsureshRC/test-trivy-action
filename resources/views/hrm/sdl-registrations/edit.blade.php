...
@section('content')
<form action="{{ route('sdl-registrations.update',$sdlRegistration->id) }}" class="mt-3" method="post">
    @csrf
    @method('put')
            ...
            <div class="row ">
                <div class="col-xxl:4 col-xl:4 col-md-4">
                    ...
                    <select name="sdl_registration" class="form-control @error('sdl_registration') is-invalid @enderror">
                        ...
                        <option value=" registered" {{ (old('sdl_registration', $sdlRegistration->sdl_registration) == 'registered') ? 'selected' : '' }}>
                           Registered
                        </option>
                        <option value="not registered" {{ (old( 'sdl_registration',$sdlRegistration->sdl_registration) == 'not registered') ? 'selected' : '' }}>
                            Not registered
                        </option>
                        <option value="registered SDL exempt" {{ (old('sdl_registration', $sdlRegistration->sdl_registration) == 'registered SDL exempt') ? 'selected' : '' }}>
                ...
                <div class="col-xxl:2 col-xl:2 col-md-2">
                    <label class="require form-label">{{ __('Effective from:') }}</label>
                        <input type="date" name="effective_from"
                            ...
                            value="{{ old('effective_from',$sdlRegistration->effective_from) }}">
