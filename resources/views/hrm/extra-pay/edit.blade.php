...
@section('page-action')
    <div>
        <a href="{{ route('payroll.index', ['employee_id' => $extraPay->employee_id, 'term' => $term]) }}" class="btn btn-rc-outline btn-sm">
...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="editForm" action="{{ route('extra-pay.update', $extraPay) }}" method="POST">
            @csrf
            @method('put')
            <div class="row">
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input class="form-control @error('amount') is-invalid @enderror" type="number"
                                        ...
                                        value="{{ old('amount', $extraPay->amount) }}"
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a class="btn btn-rc-outline"
                                    href="{{ route('payroll.index', ['employee_id' => $extraPay->employee_id]) }}">{{ __('Cancel') }}</a>
...
<script>
    $(document).ready(function() {
        $('#editForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("extra-pay.ajax-validate-update", $extraPay->id) }}',
                data: formData,
