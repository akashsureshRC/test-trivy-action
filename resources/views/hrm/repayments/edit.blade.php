...
@section('page-action')
    <div>
        <a href="{{ route('payroll.index', ['employee_id' => $repayment->employee_id, 'term' => $term]) }}" class="btn btn-rc-outline btn-sm">
...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form action="{{ route('repayments.update', $repayment) }}" method="POST" id="editForm">
            @csrf
            @method('put')
            <div class="row">
                <div class="col-md-12 col-xs-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <input type="hidden" name="term" value="{{ old('term', $term ?? $repayment->term) }}">

                            <div class="">
                                
                            <div class="d-flex justify-content-end gap-2 mt-3">
                                <a class="btn btn-rc-outline"
                                    href="{{ route('payroll.index', ['employee_id' => $repayment->employee_id]) }}">
                                    {{ __('Cancel') }}
                                </a>
...
<script>
    $(document).ready(function() {
        $('#editForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("repayments.ajax-validate-update", $repayment->id) }}',
                data: formData,
