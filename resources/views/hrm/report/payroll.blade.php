...
@push('css')
<style>
body.theme-1 .btn_tab.bg-primary.checked, body.theme-1 .btn_tab.bg-primary.active {
...
<div class="col-sm-12 col-lg-12 col-xl-12 col-md-12">
    ...
    <div class="col-sm-12">
        <x-rc-table title="{{ __('Payroll Report') }}" titleIcon="ti ti-report-money">
            <x-rc-table.content>
                    ...
                    <tbody>
                        @forelse ($employees as $key => $employee)
                            <tr>
                                        ...
                                        <td>
                                            @permission('employee show')
                                                <a class="btn btn-outline-primary"
                                                    href="{{ route('employees.show', $employee->id) }}">{{ App\Services\PayrollHelperService::employeeIdFormat($employee->employee_id) }}</a>
                                            @else
                                                <a  class="btn btn-outline-primary">{{ App\Services\PayrollHelperService::employeeIdFormat($employee->employee_id) }}</a>
                                            @endpermission
                                        </td>
                                    @else
                                        <td>--</td>
                                    @endif
                                <td>{{ $employee->firrst_name}}</td>
                                {{--<td>{{ !empty($employee->salary_type()) ?  ($employee->salary_type()) ?? '' : '' }}</td>--}}
                                 <td>{{ $employee->salary_month() ? date('M-Y', strtotime($employee->salary_month())) : '' }}</td>
                                @php
                                    $emp_total = 0;
                                ...
                                @endphp
                                <td>{{ currencyFormat($emp_total == 0 ? $employee->salary : $emp_total) }}</td>
                            </tr>
                        @empty
