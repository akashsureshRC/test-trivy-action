...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="createForm" action="{{ route('allowance-internationals.store') }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" step="0.01" min="0" name="paid_to_employee" class="form-control @error('paid_to_employee') is-invalid @enderror"
                                       value="{{ old('paid_to_employee') }}" placeholder="{{ __('Enter Amount') }}">
                                @error('paid_to_employee') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="form-group col-md-6">
                                <label class="require form-label">{{ __('Maximum Daily Deemed Amount') }}</label>
                                <input type="number" step="0.01" min="0" name="deemed_amount" class="form-control @error('deemed_amount') is-invalid @enderror"
                                       value="{{ old('deemed_amount') }}" placeholder="{{ __('Enter Amount') }}">
                                @error('deemed_amount') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="form-group col-md-6">
...
<script>
    $(document).ready(function() {
        $('#createForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("allowance-internationals.ajax-validate-store") }}',
                data: formData,
