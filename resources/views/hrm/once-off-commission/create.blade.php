...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="createForm" action="{{ route('once-off-commission.store') }}" method="POST">
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="require form-label">{{ __('Once Off Amount') }}</label>
                                <input class="form-control @error('commission_amount') is-invalid @enderror"
                                       type="number" step="0.01" min="0" name="commission_amount" value="{{ old('commission_amount') }}"
...
<script>
    $(document).ready(function() {
        $('#createForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("once-off-commission.ajax-validate-store") }}',
                data: formData,
