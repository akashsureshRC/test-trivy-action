...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="createForm" action="{{ route('company-car-operating.store') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select name="taxable_percentage" id="taxable_percentage" class="form-control @error('taxable_percentage') is-invalid @enderror">
                                    <option value="">{{ __('Select Taxable Percentage') }}</option>
                                    @foreach($taxableTypes as $taxableType)
                                        <option value="{{ $taxableType->percentage }}" {{ old('taxable_percentage') == $taxableType->percentage ? 'selected' : '' }}>
...
<script>
    $(document).ready(function() {
        $('#createForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("company-car-operating.ajax-validate-store") }}',
                data: formData,
