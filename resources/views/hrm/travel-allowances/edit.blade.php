...
@section('page-action')
    <div>
        <a href="{{ route('payroll.index', ['employee_id' => $travelAllowance->employee_id, 'term' => $term]) }}" class="btn btn-rc-outline btn-sm">
...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form action="{{ route('travel-allowances.update', $travelAllowance) }}" method="POST" id="editForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="checkbox" class="form-check-input" id="fixed_allowance" name="fixed_allowance" value="1"
                                    {{ $travelAllowance->fixed_allowance ? 'checked' : '' }}>

                                <label class="require form-label">{{ __('Fixed allowance paid regularly') }}</label>
                                <input type="number" step="0.01" id="fixed_amount" name="fixed_amount"
                                    class="form-control mt-2 {{ $travelAllowance->fixed_allowance ? '' : 'd-none' }}"
                                    value="{{ $travelAllowance->fixed_amount }}" placeholder="Enter fixed amount">
                                @error('fixed_amount')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="checkbox" class="form-check-input" id="reimbursed_expenses" name="reimbursed_expenses" value="1"
                                {{ $travelAllowance->reimbursed_expenses ? 'checked' : '' }}>

                            <label class="require form-label">{{ __('Reimbursed for expenses (petrol, garage, maintenance, etc.)') }}</label>
                            <p class="text-muted mt-2 {{ $travelAllowance->reimbursed_expenses ? '' : 'd-none' }}" id="expense_prompt">
                                You will be prompted on every payslip for the Expenses</p>
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="checkbox" class="form-check-input" id="company_petrol_card" name="company_petrol_card" value="1"
                                {{ $travelAllowance->company_petrol_card ? 'checked' : '' }}>
                            <label for="company_petrol_card">{{ __('Company Petrol Card (not paid out)') }}</label>
                            <p class="text-muted mt-2 {{ $travelAllowance->company_petrol_card ? '' : 'd-none' }}" id="petrol_card_prompt">
                                You will be prompted on every payslip for the Petrol card spend</p>
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="checkbox" class="form-check-input" id="reimbursed_per_km" name="reimbursed_per_km" value="1"
                                {{ $travelAllowance->reimbursed_per_km ? 'checked' : '' }}>
                            <label for="reimbursed_per_km">{{ __('Reimbursed per Km travelled') }}</label>

                            <input type="number" step="0.01" id="rate_per_km" name="rate_per_km"
                                class="form-control mt-2 {{ $travelAllowance->reimbursed_per_km ? '' : 'd-none' }}"
                                value="{{ $travelAllowance->rate_per_km }}" placeholder="Enter Rate per Km">
                            <p class="text-muted mt-2 {{ $travelAllowance->reimbursed_per_km ? '' : 'd-none' }}" id="km_prompt">
                                You will be prompted on every payslip for the Kms travelled</p>
                        </div>
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="checkbox" class="form-check-input" id="subject_to_20_tax" name="subject_to_20_tax" value="1"
                                {{ $travelAllowance->subject_to_20_tax ? 'checked' : '' }}>
                            <label for="subject_to_20_tax">{{ __('Only 20% Subject to Tax') }}</label>
                        </div>

                        <input type="hidden" name="employee_id" value="{{ $travelAllowance->employee_id }}">
                        <input type="hidden" name="term" value="{{ $term }}">

                        <div class="d-flex justify-content-end gap-2 mt-3">
                            <a class="btn btn-rc-outline" href="{{ route('payroll.index', ['employee_id' => $travelAllowance->employee_id]) }}">
                                {{ __('Cancel') }}
                            </a>
...
<script>
    $(document).ready(function() {
        $('#editForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("travel-allowances.ajax-validate-update", $travelAllowance->id) }}',
                data: formData,
