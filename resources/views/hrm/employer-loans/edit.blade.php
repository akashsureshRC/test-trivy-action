...
@section('page-action')
    <div>
        <a href="{{ route('payroll.index', ['employee_id' => $employer_loan->employee_id, 'term' => $term]) }}" class="btn btn-rc-outline btn-sm">
...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="editForm" action="{{ route('employer-loans.update', $employer_loan) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input class="form-control @error('interest_rate') is-invalid @enderror"
                                       type="number" step="0.01" min="0" name="interest_rate"
                                       value="{{ old('interest_rate', $employer_loan->interest_rate) }}"
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <input class="form-control @error('regular_repayment') is-invalid @enderror"
                                       type="number" step="0.01" min="0" name="regular_repayment"
                                       value="{{ old('regular_repayment', $employer_loan->regular_repayment) }}"
                                       placeholder="{{ __('Enter Amount') }}">
                            ...
                            <div class="form-group col-md-6">
                                <div class="form-check">
                                    ...
                                    <input type="checkbox" class="form-check-input" name="calculate_interest_benefit" value="1"
                                        {{ old('calculate_interest_benefit', $employer_loan->calculate_interest_benefit) ? 'checked' : '' }} id="calculate_interest_benefit">
                            ...
                            <div class="form-group col-md-6" id="interest_benefit_amount_wrapper" style="display: none;">
                                ...
                                <input class="form-control @error('interest_benefit_amount') is-invalid @enderror"
                                       ...
                                       readonly
                                       value="{{ old('interest_benefit_amount', $employer_loan->interest_benefit_amount ?? 0) }}"
                                       placeholder="{{ __('Enter Amount') }}">
                        ...
                        <input type="hidden" name="employee_id" value="{{ $employer_loan->employee_id }}">
                        <input type="hidden" name="term" value="{{ $term }}">

                        <div class="d-flex justify-content-end gap-2 mt-3">
                            <a class="btn btn-rc-outline" href="{{ route('payroll.index', ['employee_id' => $employer_loan->employee_id, 'term' => $term]) }}">
                                {{ __('Cancel') }}
                            </a>
...
<script>
    ...
    $('#calculate_interest_benefit').on('change', toggleInterestBenefitAmount);
        ...
        $('#editForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("employer-loans.ajax-validate-update", $employer_loan->id) }}',
                data: formData,
