...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="createForm" action="{{ route('garnishee.store') }}" method="POST">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="form-group col-md-6">
                            ...
                            <input class="form-control" type="text" value="{{ $employee->first_name }} {{ $employee->last_name }}" readonly>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="require form-label">{{ __('Installment') }}</label>
                                <input class="form-control @error('installment') is-invalid @enderror"
                                       type="number" step="0.01" min="0" name="installment" value="{{ old('installment') }}"
                        ...
                        <p class="text-muted">{{ __('The Increase balance may be entered under Payslip Inputs') }}</p>
...
<script>
    $(document).ready(function() {
        $('#createForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("garnishee.ajax-validate-store") }}',
                data: formData,
