...
@section('page-action')
    <div>
        <a href="{{ route('payroll.index', ['employee_id' => $garnishee->employee_id, 'term' => $term]) }}" class="btn btn-rc-outline btn-sm">
...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="editForm" action="{{ route('garnishee.update', $employee) }}" method="POST">
            @csrf
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input class="form-control @error('installment') is-invalid @enderror" type="number"
                                        step="0.01" min="0" name="installment"
                                        value="{{ old('installment', $garnishee->installment) }}"
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                <a class="btn btn-rc-outline"
                                    href="{{ route('payroll.index', ['employee_id' => $garnishee->employee_id]) }}">
                                    {{ __('Cancel') }}
                                </a>
...
<script>
    $(document).ready(function() {
        $('#editForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("garnishee.ajax-validate-update", $garnishee->id) }}',
                data: formData,
