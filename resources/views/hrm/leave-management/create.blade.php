...
@section('page-title')
    {{ __('Create Leave Module') }}
@endsection

@section('page-breadcrumb')
    {{ __('Leave Management') }}, {{ __('Create') }}
@endsection


@push('css')
<link rel="stylesheet" href="{{ asset('Modules/Pos/Resources/assets/css/custom.css') }}">
<style>
    .time-fields, .inventory-fields {
        display: none;
    }
</style>
@endpush
...
@section('content')
    @if (Auth::check())
        <form method="POST" action="{{ route('hrm.leave-management.store') }}" class="mt-3">
            @csrf
            <div class="row">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" name="leave_name"
                                        ...
                                        value="{{ old('leave_name') }}" placeholder="Enter Leave Name" onkeypress="blockNumbers(event)">
                                    @error('leave_name')
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="number" name="cycle_length"
                                        ...
                                        value="{{ old('cycle_length') }}" placeholder="Enter number of months"
                                        min="1" step="1">
                                    @error('cycle_length')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="cycle_start_type" id="cycle_start_type"
                                        ...
                                        <option value="appointment"
                                            {{ old('cycle_start_type') == 'appointment' ? 'selected' : '' }}>Appointment
                                            Date</option>
                                        <option value="january"
                                            {{ old('cycle_start_type') == 'january' ? 'selected' : '' }}>01 January
                                        </option>
                                        <option value="custom" {{ old('cycle_start_type') == 'custom' ? 'selected' : '' }}>
                                            Custom</option>
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="visible_for"
                                        ...
                                        <option value="everyone" {{ old('visible_for') == 'everyone' ? 'selected' : '' }}>
                                            Everyone</option>
                                        <option value="employees"
                                            {{ old('visible_for') == 'employees' ? 'selected' : '' }}>Employees with a
                                            recent, current or future entitlement</option>
                                    </select>
                                    @error('visible_for')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                ...
                                {{-- <div class="form-group col-md-6">
                                    ...
                                    <input type="checkbox" class="form-check-input" id="set_min_balance_rule" name="set_min_balance_rule"
                                        value="1" {{ old('set_min_balance_rule') ? 'checked' : '' }}>
                                    {{ __('Set Minimum Balance Rule') }}
                                </div>
...
@enderror
                                    ...
                                    <div class="form-group col-md-6">
                                        <div class="form-check">
                                            <input type="hidden" name="set_min_balance_rule" value="0">
                                            <input type="checkbox" class="form-check-input" id="set_min_balance_rule" name="set_min_balance_rule" {{ old('set_min_balance_rule') ? 'checked' : '' }} value="1">
                                            <label class="form-check-label" for="set_min_balance_rule">
                                                {{ __('Set Minimum Balance Rule') }}
                                            </label>
                                        </div>
                                    </div>
                                    ...
                                    <div id="min_balance_wrapper" class="row" style="display: none;">
                                        <div class="form-group col-md-6">
                                            <label class="form-label">{{ __('Minimum Balance') }}</label>
                                            <input type="number" step="0.01" name="minimum_balance"
                                                   class="form-control @error('minimum_balance') is-invalid @enderror" 
                                                   value="{{ old('minimum_balance') }}">
                                            @error('minimum_balance')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select name="allow_rule_override" class="form-control">
                                        <option value="not allowed"
                                            {{ old('allow_rule_override') == 'not allowed' ? 'selected' : '' }}>Not Allowed
                                        </option>
                                        <option value="admins"
                                            {{ old('allow_rule_override') == 'admins' ? 'admins' : '' }}>Admins &amp; Leave
                                            Admins</option>
                                        <option value="approvers admin"
                                            {{ old('allow_rule_override') == 'approvers admin' ? 'approvers admin' : '' }}>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const checkbox = document.getElementById('set_min_balance_rule');
            const wrapper = document.getElementById('min_balance_wrapper');
    
            function toggleMinBalance() {
                if (checkbox.checked) {
            ...
            checkbox.addEventListener('change', toggleMinBalance);
            toggleMinBalance(); // Run on page load
        });
    </script>
     <script>
