...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form id="createForm" action="{{ route('termination-lumps.store') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select name="directive_income_source_code" class="form-control @error('directive_income_source_code') is-invalid @enderror">
                                    <option value="">{{ __('Select Source Code') }}</option>
                                    <option value="3901" {{ old('directive_income_source_code') == '3901' ? 'selected' : '' }}>{{ __('3901 - Severance / Gratitude Benefits') }}</option>
                                    <option value="3907" {{ old('directive_income_source_code') == '3907' ? 'selected' : '' }}>{{ __('3907 - Other Lump Sums') }}</option>
                                </select>
                                @error('directive_income_source_code') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="form-group col-md-6">
                                ...
                                <input type="number" step="0.01" min="0" name="amount_of_tax_to_deduct" class="form-control @error('amount_of_tax_to_deduct') is-invalid @enderror"
                                       value="{{ old('amount_of_tax_to_deduct', 0) }}">
                                @error('amount_of_tax_to_deduct') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="form-group col-md-6">
...
<script>
    $(document).ready(function() {
        $('#createForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("termination-lumps.ajax-validate-store") }}',
                data: formData,
