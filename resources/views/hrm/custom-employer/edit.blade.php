...
@section('content')
    ...
    <body>
        ...
        <div class="container mt-5">
            <div class="white-section"> <!-- White background section -->
                ...
                <form action="{{ route('custom-employer.update',$customemployerContribution->id) }}" class="mt-3" method="post">
                    @csrf
                    @method('put')
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="name"
                                class="form-control @error('name') is-invalid @enderror" value="{{ old('name',$customemployerContribution->name) }}">
                            @error('name')
                    ...
                    <br>
                      ...
                      <div class="row mb-3">
                        ...
                        <select id="input_type" name="input_type" class="form-control" required>
                          ...
                          <option value="fixed_amount"{{ old('input_type',$customemployerContribution->input_type) == 'fixed_amount' ? 'selected' : '' }}>Fixed Amount</option>
                          <option value="enter_amount_per_employee" {{ old('input_type',$customemployerContribution->input_type) == 'fixed_amount' ? 'selected' : '' }}>Enter Amount Per Employee</option>
                          <option value="different_on_every_payslip" {{ old('input_type',$customemployerContribution->input_type) == 'enter_amount_per_employee' ? 'selected' : '' }}>Different on Every Payslip</option>
                          <option value="once_off"{{ old('input_type',$customemployerContribution->input_type) == 'once_off' ? 'selected' : '' }}>Once-off for Specified Payslips</option>
                          <option value="hourly_rate_factor_hours"{{ old('input_type',$customemployerContribution->input_type) == 'hourly_rate_factor_hours' ? 'selected' : '' }}>Hourly rate * factor * hours</option>
                          <option value="custom_rate_quantity"{{ old('input_type',$customemployerContribution->input_type) == 'custom_rate_quantity' ? 'selected' : '' }}>Custom rate * quantity</option>
                          <option value="percentage_income"{{ old('input_type',$customemployerContribution->input_type) == 'percentage_income' ? 'selected' : '' }}>% of Income</option>
                          <option value="formula"{{ old('input_type',$customemployerContribution->input_type) == 'formula' ? 'selected' : '' }}>Formula</option>
                          <option value="monthly"{{ old('input_type',$customemployerContribution->input_type) == 'monthly' ? 'selected' : '' }}>Monthly (for non-monthly employees)</option>
                        </select>
                      </div></div>
                        ...
                        <div id="fixed_amount_section" style="display: none;">
                            <div class="mb-3">
                                ...
                                <input type="text" id="amount_fixed" name="amount"
                                class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount',$customemployerContribution->amount) }}">
                            @error('amount')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            </div>
                      ...
                      <div id="hourly_rate_section" style="display: none;">
                        <div class="mb-3">
                          ...
                          <input type="text" id="rate_factor" name="rate_factor"
                                class="form-control @error('rate_factor') is-invalid @enderror" value="{{ old('rate_factor',$customemployerContribution->rate_factor) }}">
                            @error('rate_factor')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                      ...
                      <div id="custom_rate_section" style="display: none;">
                        ...
                        <div class="mb-3" id="custom_rate_input_div">
                          ...
                          class="form-control @error('custom_rate') is-invalid @enderror" value="{{ old('custom_rate',$customemployerContribution->custom_rate) }}">
                      @error('custom_rate')
                          <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                        </div>
                      ...
                      <div id="percentage_income_section" style="display: none;">
                        <div class="mb-3">
                         ...
                         <!-- <input type="number" step="0.01" id="percentage_income" name="percentage_income" class="form-control">-->
                          <input type="text" id="percentage_income" name="percentage_income"
                          class="form-control @error('percentage_income') is-invalid @enderror" value="{{ old('percentage_income',$customemployerContribution->percentage_income) }}">
                      @error('percentage_income')
                          <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                        </div>
                        <div class="mb-3">
                      ...
                      <div id="formula_section" style="display: none;">
                        <div class="mb-3">
                          ...
                          class="form-control @error('formula') is-invalid @enderror" value="{{ old('formula',$customemployerContribution->formula) }}">
                      @error('formula')
                          <div class="invalid-feedback">{{ $message }}</div>
                      @enderror
                        </div>
                      ...
                      <div id="monthly_section" style="display: none;">
                        <div class="mb-3">
                          ...
                          <input type="text"  id="monthly_amount" name="monthly_amount"
                          class="form-control @error('monthly_amount') is-invalid @enderror" value="{{ old('monthly_amount',$customemployerContribution->monthly_amount) }}">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Run function on page load (if there's a pre-selected value)
            toggleSections();
    
            // Add event listener for dropdown changes
            inputTypeSelect.addEventListener('change', toggleSections);
    
            // Handle custom rate checkbox toggle for "Custom rate * quantity"
            var differentRateCheckbox = document.getElementById('different_rate_employee');
            if (differentRateCheckbox) {
                differentRateCheckbox.addEventListener('change', function() {
                    var customRateInputDiv = document.getElementById('custom_rate_input_div');
                    customRateInputDiv.style.display = this.checked ? 'none' : 'block';
                });
            }
        });
    </script>
