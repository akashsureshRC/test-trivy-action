...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <form action="{{ route('pension-fund.store') }}" method="POST" id="createForm">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select class="form-control @error('pension') is-invalid @enderror" name="pension" id="pension">
                                <option value="">{{ __('Select Calculation') }}</option>
                                <option value="fixed_amount" {{ old('pension') == 'fixed_amount' ? 'selected' : '' }}>{{ __('Fixed Amount') }}</option>
                                <option value="percentage_rfi" {{ old('pension') == 'percentage_rfi' ? 'selected' : '' }}>{{ __('% of Retirement Funding Income') }}</option>
                            </select>
                            @error('pension')
                        ...
                        <!-- Fixed Amount Inputs -->
                        <div id="fixed_contribution_section" style="{{ old('pension') == 'fixed_amount' ? 'display:block;' : 'display:none;' }}">
                            <div class="form-group col-md-6">
                        ...
                        <div id="percentage_rfi_section" style="{{ old('pension') == 'percentage_rfi' ? 'display:block;' : 'display:none;' }}">
                            <div class="form-group col-md-6">
...
<script>
    $(document).ready(function() {
        $('#createForm').on('submit', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '{{ route("pension-fund.ajax-validate-store") }}',
                data: formData,
