...
@section('page-title')
{{ __('Edit Entitlement Policy') }}
@endsection

@section('page-breadcrumb')
{{ __('Leave Management') }}, {{ __('Entitlement Policies') }}, {{ __('Edit') }}
@endsection

@section('page-action')
<div>
    <a href="{{ route('entitlement-policies.index', ['leave' => $leave->id]) }}" class="btn btn-sm btn-rc-icon"
...
<div class="row">
    <div class="col-sm-12">
        @if (Auth::check())
            <form action="{{ route('entitlement-policies.update', $entitlementPolicy->id) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" value="{{ $leave ? $leave->leave_name : '' }}" readonly>
                                    <input type="hidden" name="leave_management_id" id="leave_management_id" value="{{ $leave ? $leave->id : '' }}">
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    <div class="form-check">
                                        ...
                                        <input type="checkbox" class="form-check-input" id="use_hours_worked" name="use_hours_worked" value="1"
                                            {{ old('use_hours_worked', $entitlementPolicy->use_hours_worked) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="use_hours_worked">
                                            {{ __('Use hours worked for accrual (casual staff)') }}
                                        </label>
                                    </div>

                                    <div id="hours_worked_section" class="mt-2 ps-4" style="{{ old('use_hours_worked', $entitlementPolicy->use_hours_worked) ? '' : 'display:none;' }}">
                                        <div class="form-group mb-2">
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <input type="number" name="hours_per_leave"
                                                    ...
                                                    value="{{ old('hours_per_leave', $entitlementPolicy->hours_per_leave) }}">
                                        ...
                                        <div class="form-check">
                                            ...
                                            <input type="checkbox" class="form-check-input" name="paid_leave_contributes" id="paid_leave_contributes" value="1"
                                                {{ old('paid_leave_contributes', $entitlementPolicy->paid_leave_contributes) ? 'checked' : '' }}>
                                ...
                                <div id="standard_accrual_section" style="{{ old('use_hours_worked', $entitlementPolicy->use_hours_worked) ? 'display:none;' : '' }}">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="number" name="default_entitlement" id="default_entitlement"
                                            ...
                                            placeholder="{{ __('Days') }}"
                                            value="{{ old('default_entitlement', $entitlementPolicy->default_entitlement) }}">
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <div class="d-flex align-items-center gap-2">
                                            <input type="number" name="entitlement_after_months" id="entitlement_after_months"
                                                ...
                                                value="{{ old('entitlement_after_months', $entitlementPolicy->entitlement_after_months) }}">
                                    ...
                                    <div class="form-group mb-3">
                                        <div class="form-check">
                                            ...
                                            <input type="checkbox" class="form-check-input" name="use_upfront_accrual" id="use_upfront_accrual" value="1"
                                                {{ old('use_upfront_accrual', $entitlementPolicy->use_upfront_accrual) ? 'checked' : '' }}>
                                ...
                                <div class="form-group mb-3">
                                    <div class="form-check">
                                        ...
                                        <input type="checkbox" class="form-check-input" id="use_custom_name" name="use_custom_name" value="1"
                                            {{ old('use_custom_name', $entitlementPolicy->use_custom_name) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="use_custom_name">{{ __('Use custom name?') }}</label>
                                    </div>
                                    <div id="custom_name_wrapper" class="mt-2 ps-4" style="{{ old('use_custom_name', $entitlementPolicy->use_custom_name) ? '' : 'display:none;' }}">
                                        <input type="text" id="custom_name_input" name="custom_name"
                                            ...
                                            value="{{ old('custom_name', $entitlementPolicy->custom_name) }}"
                                            {{ old('use_custom_name', $entitlementPolicy->use_custom_name) ? 'required' : '' }}>
                                        @error('custom_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group mb-3">
                                    <div class="form-check">
                                        ...
                                        <input type="checkbox" class="form-check-input" id="allow_carry_forward" name="allow_carry_forward" value="1"
                                            {{ old('allow_carry_forward', $entitlementPolicy->allow_carry_forward) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="allow_carry_forward">
                                            {{ __('Allow leave to be carried forward to next cycle?') }}
                                        </label>
                                    </div>

                                    <div id="carry_forward_section" class="mt-3 ps-4" style="{{ old('allow_carry_forward', $entitlementPolicy->allow_carry_forward) ? '' : 'display:none;' }}">
                                        <div class="form-group mb-3">
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <input type="number" name="carry_forward_expiry_months" id="carry_forward_expiry_months"
                                                    ...
                                                    style="width: 100px;" placeholder="{{ __('Months') }}"
                                                    value="{{ old('carry_forward_expiry_months', $entitlementPolicy->carry_forward_expiry_months) }}">
                                                <span class="text-muted">{{ __('months') }}</span>
                                            </div>
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <select name="limit_type" id="limit_type"
                                                ...
                                                <option value="no" {{ old('limit_type', $entitlementPolicy->limit_type) == 'no' ? 'selected' : '' }}>
                                                    {{ __('No limit') }}
                                                </option>
                                                <option value="percentage" {{ old('limit_type', $entitlementPolicy->limit_type) == 'percentage' ? 'selected' : '' }}>
                                                    {{ __('Percentage of balance') }}
                                                </option>
                                                <option value="percentage_entitlement" {{ old('limit_type', $entitlementPolicy->limit_type) == 'percentage_entitlement' ? 'selected' : '' }}>
                                                    {{ __('Percentage of entitlement') }}
                                                </option>
                                                <option value="fixed" {{ old('limit_type', $entitlementPolicy->limit_type) == 'fixed' ? 'selected' : '' }}>
                                        ...
                                        <div id="limit_value_input" class="form-group mb-3" style="{{ old('limit_type', $entitlementPolicy->limit_type) == 'no' ? 'display:none;' : '' }}">
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                <input type="number" id="limit_value" name="limit_value"
                                                    ...
                                                    value="{{ old('limit_value', $entitlementPolicy->limit_value) }}">
                                                <span id="unitLabel" class="text-muted">
                                                    @if(in_array(old('limit_type', $entitlementPolicy->limit_type), ['percentage', 'percentage_entitlement']))
                                                    %
                                                    @elseif(old('limit_type', $entitlementPolicy->limit_type) == 'fixed')
                                                    {{ __('days') }}
                                                    @endif
                                                </span>
                                            </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div id="noRulesMessage" class="text-center py-4 {{ count($rules) > 0 ? 'd-none' : '' }}">
                        ...
                        <div id="rulesContainer">
                            ...
                            <div class="row mb-2 align-items-center rule-row">
                                <div class="col-md-3">
                                    <input type="number" name="first_cycle[]" class="form-control" value="{{ $rule['first_cycle'] }}" placeholder="{{ __('First Cycle') }}">
                                </div>
                                <div class="col-md-3">
                                    <input type="number" name="last_cycle[]" class="form-control" value="{{ $rule['last_cycle'] }}" placeholder="{{ __('Last Cycle') }}">
                                </div>
                                <div class="col-md-3">
                                    <input type="number" name="entitlement[]" class="form-control" value="{{ $rule['entitlement'] }}" placeholder="{{ __('Entitlement') }}">
                                </div>
                                <div class="col-md-3">
                ...
                <div class="card">
                    <div class="card-body d-flex justify-content-end gap-2">
                        <a href="{{ route('entitlement-policies.index', ['leave' => $leave->id]) }}" class="btn btn-rc-outline">{{ __('Cancel') }}</a>
