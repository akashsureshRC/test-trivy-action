    ...
    <body>
        ...
        <div class="container mt-5">
            <div class="white-section"> <!-- White background section -->
                ...
                <form action="{{ route('custom-reimbursements.update',$customReimbursement->id) }}" class="mt-3" method="post">
                    @csrf
                    @method('put')
                    <div class="row mb-3">
                        ...
                        <div class="col-sm-10">
                            <input type="text" name="name"
                                class="form-control @error('name') is-invalid @enderror" value="{{ old('name',$customReimbursement->name) }}">
                            @error('name')
                    ...
                    <div class="row mb-3">
                        ...
                        <select class="form-control" id="input_type" name="input_type">
                            <option value="different_on_every_payslip" {{ old('input_type',$customReimbursement->input_type) == 'different_on_every_payslip' ? 'selected' : '' }}>Different on Every Payslip</option>
                            <option value="once_off" {{ old('input_type',$customReimbursement->input_type) == 'once_off' ? 'selected' : '' }}>Once Off for Specified Payslips</option>
                            <option value="custom_rate_quantity" {{ old('input_type',$customReimbursement->input_type) == 'custom_rate_quantity' ? 'selected' : '' }}>Custom Rate * Quantity</option>
                        </select>
                    </div></div>
                    ...
                    <div class="mb-3" id="custom_rate_container" style="display: none;">
                        ...
                        <input type="number"id="custom_rate" name="custom_rate"  step="0.01"
                                class="form-control @error('custom_rate') is-invalid @enderror" value="{{ old('custom_rate',$customReimbursement->custom_rate) }}">
                            @error('custom_rate')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    </div>
