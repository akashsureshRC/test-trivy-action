...
$(document).on('click', 'a[data-ajax-popup="true"], button[data-ajax-popup="true"], div[data-ajax-popup="true"]', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Disable browser native validation on modal forms
            $('#commonModal form').attr('novalidate', 'novalidate');
            summernote();
            taskCheckbox();
...
$(document).on('click', 'a[data-ajax-popup-over="true"], button[data-ajax-popup-over="true"], div[data-ajax-popup-over="true"]', function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Disable browser native validation on modal-over forms
            $('#commonModalOver form').attr('novalidate', 'novalidate');
            summernote();
            taskCheckbox();
        },
        error: function (xhr) {
...
$(document).on("submit", "#commonModal form", function (e) {
    ...
    // Skip global handler if form has its own custom submit handler
    if (form.attr('data-custom-submit') === 'true') return;

    // Custom validation - replaces browser native validation with styled messages
    ...
    form.find('[required]').each(function() {
        ...
        // Skip hidden fields (except selects and hidden inputs)
        if ($field.is(':hidden') && tagName !== 'select' && $field.attr('type') !== 'hidden') return;

        var isEmpty = !value || (typeof value === 'string' && value.trim() === '');

        if (isEmpty) {
            isValid = false;
            var label = $field.closest('.form-group').find('.form-label, label').first().text().replace('*', '').trim();
            if (!label) label = $field.attr('placeholder') || $field.attr('name') || 'This';
            $field.after('<p class="text-danger js-validation-error">' + label + ' field is required.</p>');
        }

        // Email format validation
        if (!isEmpty && $field.attr('type') === 'email') {
            var emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            if (!emailRegex.test(value.trim())) {
                isValid = false;
                $field.after('<p class="text-danger js-validation-error">Please enter a valid email address.</p>');
            }
        }

        // Min length validation
        var minlength = $field.attr('minlength');
        if (!isEmpty && minlength && value.length < parseInt(minlength)) {
            isValid = false;
            var label = $field.closest('.form-group').find('.form-label, label').first().text().replace('*', '').trim();
            if (!label) label = $field.attr('name') || 'This';
            $field.after('<p class="text-danger js-validation-error">' + label + ' must be at least ' + minlength + ' characters.</p>');
        }
    });
...
$(document).on("submit", "#commonModalOver form", function (e) {
    ...
    // Custom validation for modal-over forms
    form.find('.js-validation-error').remove();
    var isValid = true;
    ...
    form.find('[required]').each(function() {
        var $field = $(this);
        var value = $field.val();
        if ($field.is(':hidden') && !$field.is('select')) return;
...
// Global: Clear validation error messages on input/change
$(document).on('input change', '[required]', function() {
    $(this).siblings('.js-validation-error').remove();
});

// Global: Disable browser validation on all forms
$(document).ready(function() {
    $('form').attr('novalidate', 'novalidate');
});

// Global: Validate full-page forms (not in modals) with required fields
$(document).on('submit', 'form', function(e) {
    var form = $(this);
    // Skip modal forms (handled by their own handlers above)
    if (form.closest('#commonModal, #commonModalOver').length) return;
    // Skip forms without required fields
    if (form.find('[required]').length === 0) return;
    // Skip custom handled forms
    if (form.attr('data-custom-submit') === 'true') return;
    ...
    form.find('[required]').each(function() {
        ...
        if (!isEmpty && minlength && value.length < parseInt(minlength)) {
            isValid = false;
            var label2 = $field.closest('.form-group').find('.form-label, label').first().text().replace('*', '').trim();
            if (!label2) label2 = $field.attr('name') || 'This';
            $field.after('<p class="text-danger js-validation-error">' + label2 + ' must be at least ' + minlength + ' characters.</p>');
        }
    });
...
function common_bind(selector = "body") {
    ...
    if ($(".jscolor").length && typeof jscolor !== 'undefined') {
...
var Charts = (function () {
    ...
    function rcEscapeHtml(value) {
        return $('<div>').text(value || '').html();
    }

    function openRcConfirmDialog(title, text) {
        return Swal.fire({
            html: '<div class="rc-confirm-body">'
                + '<div class="rc-confirm-icon" aria-hidden="true"><i class="ti ti-alert-triangle"></i></div>'
                + '<h2 class="rc-confirm-heading">' + rcEscapeHtml(title) + '</h2>'
                + '<p class="rc-confirm-text">' + rcEscapeHtml(text) + '</p>'
                + '</div>',
    ...
    $('.remove_workspace').click(function (event) {
        ...
        openRcConfirmDialog('Are you sure?', 'This action can not be undone. Do you want to continue?').then((result) => {
    ...
    $(document).on('click', '.show_confirm', function (event) {
        ...
        openRcConfirmDialog(title, text).then((result) => {
            if (result.isConfirmed) {
                form.submit();
            }
        });
